...
export function CluesManager({
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const loadedClues = await loadClues(activeCampaignId);
  ...
  try {
    ...
    await saveClueToSupabase(activeCampaignId, clueToAdd);
  } catch (error) {
  ...
  const toggleDiscovered = async (id: string) => {
    ...
    try {
      // Passa l'intero oggetto - il servizio si occupa del mapping automatico
      await saveClueToSupabase(activeCampaignId, updatedClue);
    } catch (error) {
