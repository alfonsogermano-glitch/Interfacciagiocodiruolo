          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                onClick={onReroll}
                aria-label="Ritira"
                className="mt-auto rounded-md border border-[var(--dash-border)] bg-[var(--dash-surface)] p-1 text-[var(--dash-muted)] transition-colors hover:bg-[var(--dash-surface-2)] hover:text-[var(--dash-text)]"
              >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="shrink-0 text-right">
              ...
              <div className="mt-0.5 text-xl font-bold leading-none text-[var(--dash-text-strong)]">
                {formatPrimaryRollResult(result)}
              </div>
            </div>
          </div>

          <div className="mt-1 break-words whitespace-pre-wrap rounded bg-[var(--dash-input)] px-1.5 py-0.5 font-mono text-[11px] leading-tight text-[var(--dash-text)]">
            {result.formulaText}
          </div>
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            {result.diceGroups.flatMap((group) =>
              group.rolls.map((die) => {
                const tooltip = `${die.source === 'explosion' ? 'Rilancio esplosivo' : `d${die.sides}`}${die.active ? '' : ' - escluso'}`;
                return (
                  <Tooltip key={die.id}>
                    <TooltipTrigger asChild>
                      <span
                        className={`inline-flex items-center gap-0.5 rounded border px-1.5 py-0.5 text-[11px] leading-none ${
                          die.active
