...
import { Ban, Clipboard, Copy, Eye, EyeOff, Gauge, Pencil, Trash2 } from 'lucide-react';
...
const DANGER = 'flex w-full items-center gap-2 rounded-md px-2.5 py-1.5 text-left text-xs text-[var(--dash-danger-strong)] transition-colors hover:bg-[var(--dash-danger)] hover:text-[var(--dash-text-strong)]';
...
export function NotePointsMenu({ editor, editable }: { editor: Editor; editable: boolean }) {
  ...
  const placed = placeFloatingNoteUI({ left: request.x, right: request.x + 2, top: request.y, bottom: request.y }, 224, 244, 6);
  return createPortal(
