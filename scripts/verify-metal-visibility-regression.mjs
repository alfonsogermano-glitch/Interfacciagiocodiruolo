...
assert.ok(materials.includes("globalCompositeOperation = 'lighten'"), 'Face repair must repaint the photo under the labels without covering them');
assert.ok(materials.includes('sampledFaceLuminance(image) >= 0.3'), 'Face repair must only touch dark faces and leave correct ones alone');
