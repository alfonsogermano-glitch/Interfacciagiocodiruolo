...
assert.ok(history.includes('Pulisci')); assert.ok(/scrollContainer\.scrollTop\s*=\s*scrollContainer\.scrollHeight/.test(history)); assert.ok(history.includes('max-h-[58vh]')); assert.ok(history.includes('min-w-[18rem]')); assert.ok(history.includes('max-w-[min(20rem,calc(100vw-8rem))]')); assert.ok(history.includes('<DiceQuickRollFloating />'));
...
assert.ok(/<StandardDieResult[\s\S]{0,220}appearance=\{historyStandardAppearance\}/.test(card), 'standard history results must use the appearance snapshot stored on the roll group');
