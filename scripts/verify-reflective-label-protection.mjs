...
assert.ok(source.includes('function protectReflectiveDiceLabelFromLighting'), 'Obsidian needs a dedicated label-lighting shield');
assert.ok(source.includes('shouldProtectReflectiveDiceLabel'), 'reflective label protection must be centrally scoped');
assert.ok(source.includes("return descriptor.appearance.skinId === 'obsidian';"), 'label shield must remain limited to Obsidian');
...
assert.ok(source.includes('texture2D(reflectiveLabelMask, ${uvVarying}).a'), 'shader must sample the anti-aliased alpha from the dedicated label mask');
assert.ok(source.includes('texture2D(map, ${uvVarying}).rgb'), 'protected pixels must reuse the original user-colored face texel');
...
console.log('Reflective Obsidian label lighting protection verification passed.');
