...
assert.ok(materials.includes('const CUSTOM_FACE_STANDARD_CONTENT_RATIO = 1 / 3;'),'non-d4 custom art must match the standard dice printable area instead of filling the whole texture');
...
assert.ok(configurator.includes('className="h-6 w-6 object-contain"'),'uploaded-image preview must contain and center the whole image');
