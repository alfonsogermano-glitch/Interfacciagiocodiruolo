...
assert.ok(textures.includes('`${appearance.skinId}:${textureScale}:${readiness}`'), 'Poison ready and placeholder descriptors must use separate cache keys');
assert.ok(textures.includes('`hollowgate-${appearance.skinId}-${textureScale}-${readiness}`'), 'Renderer-facing Poison texture names must differ across readiness phases');
