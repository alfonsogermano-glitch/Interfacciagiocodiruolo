...
assert.match(categoryIcon, /left:\s*iconName === 'Cog' \? '-0\.5em' : '-0\.22em'[\s\S]*bottom:\s*'-0\.3em'[\s\S]*opacity:\s*'0\.5'[\s\S]*pointerEvents:\s*'none'/, 'category icon visual axes must cross the midpoint of the lower-left corner curve');
