...
assert.match(dice, /noteCustomDieQuantity[\s\S]*fontSize: '2em'[\s\S]*noteCustomDieFace[\s\S]*width: '2rem'[\s\S]*height: '2rem'[\s\S]*lineHeight: 0[\s\S]*CustomDieLibraryIcon/, 'the widget must place the quantity before a fixed-size Custom face container matching the edit picker alignment');
