...
assert.ok(textures.includes("context.filter = 'brightness(1.16) saturate(1.16) contrast(1.14)'"), 'Ice photo faces must receive the brighter high-contrast photographic polish');
assert.ok(textures.includes('context.globalAlpha = 0.08;'), 'Ice photo faces must receive a restrained cold screen lift');
assert.ok(textures.includes("context.fillStyle = '#e6fbff';"), 'Ice photo faces must use the brighter cold screen tint');
assert.ok(textures.includes("bump.filter = 'grayscale(1) contrast(1.95) brightness(.96)'"), 'Ice bump must preserve stronger crystalline micro-detail');
