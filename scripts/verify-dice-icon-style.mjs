...
for (const sides of diceSides) {
  ...
  assert.ok(width >= 192 && height >= 192, `${assetName} must be high-resolution enough for crisp toolbar rendering`);
  assert.ok(colorType === 4 || colorType === 6, `${assetName} must preserve transparency around the rendered die`);
}
