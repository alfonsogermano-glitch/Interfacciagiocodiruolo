...
assert.ok(data.includes("LIGHTNING_TEXTURE_CHUNK_5.replace(\n  'VYX5wW2NtoaU4',\n  'VYX5wW2NatoaU4',\n)"), 'Lightning assembly must restore the transport-dropped chunk 5 base64 character deterministically');
assert.ok(data.includes("LIGHTNING_TEXTURE_CHUNK_6.replace(\n  '7UfxYISE8N',\n  '7UfxISE8N',\n)"), 'Lightning assembly must remove the transport-inserted chunk 6 base64 character deterministically');
assert.ok(data.includes('LIGHTNING_TEXTURE_CHUNK_5_APPROVED') && data.includes('LIGHTNING_TEXTURE_CHUNK_6_APPROVED'), 'Lightning source must assemble both normalized approved shard payloads');
...
const encodedChunks = chunks.map((content, index) => {
  ...
  let encodedChunk = match[1];
  if (index === 5) encodedChunk = encodedChunk.replace('VYX5wW2NtoaU4', 'VYX5wW2NatoaU4');
  if (index === 6) encodedChunk = encodedChunk.replace('7UfxYISE8N', '7UfxISE8N');
  assert.equal(
