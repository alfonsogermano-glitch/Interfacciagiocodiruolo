...
const assetUrl = new URL('src/app/components/session/dice/assets/stoneTexture.webp', root);
...
assert.ok(fs.existsSync(dataUrl), 'Stone texture URL module must exist');
...
assert.equal(image.length, 72_604, 'Stone texture must match the optimized 448px WebP payload');
...
assert.ok(data.includes("import stoneTextureUrl from './assets/stoneTexture.webp';"), 'Stone must expose the bundled WebP through Vite');
assert.ok(data.includes('export const STONE_TEXTURE_SOURCE_DATA_URL = stoneTextureUrl;') && data.includes('export const STONE_TEXTURE_DATA_URL = stoneTextureUrl;'), 'Stone 2D and 3D must share the same source');
...
assert.ok(textures.includes("import { STONE_TEXTURE_SOURCE_DATA_URL } from './stoneTextureData.ts';") && textures.includes("createTextureImage(STONE_TEXTURE_SOURCE_DATA_URL, 'stone')"), '3D Stone must preload the bundled photograph');
...
assert.ok(ci.includes('node scripts/verify-stone-photo-asset.mjs'), 'CI must run the Stone photographic regression test');

console.log('Stone photographic asset, 2D/3D integration, matte material, readiness cache, labels and mineral effects verification passed.');
