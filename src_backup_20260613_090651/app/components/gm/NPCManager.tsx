...
export function NPCManager({
  ...
  useEffect(() => {
    ...
    loadEnvironmentReferences(DEFAULT_CAMPAIGN_ID)
      .then(references => {
        if (cancelled) return;
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const loadedNPCs = await loadNPCs(DEFAULT_CAMPAIGN_ID);
  ...
  const saveNPC = async () => {
    if (isCreating && draftNPC) {
      ...
      try {
        ...
        await saveNPCToSupabase(DEFAULT_CAMPAIGN_ID, draftNPC);
      } catch (error) {
    ...
    if (selectedNPC) {
      try {
        // Passa l'intero oggetto - il servizio si occupa del mapping automatico
        await saveNPCToSupabase(DEFAULT_CAMPAIGN_ID, selectedNPC);
      } catch (error) {
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <div className="space-y-4 overflow-visible lg:col-span-1">
        <div className="relative z-20 flex items-center justify-between overflow-visible">
          <h2 className="text-[var(--dash-text)]">PNG</h2>
        ...
        <div className="space-y-3 rounded-xl border border-[var(--dash-border-soft)] bg-[var(--dash-surface)] px-4 py-3">
          <div>
            ...
            >
              ...
              {adventures
                .filter(adventure => adventure.campaignId === DEFAULT_CAMPAIGN_ID)
                .map(adventure => (
...
function FreschezzaBoxesEditor({
  current,
  max,
  crucialBoxes,
  onUpdate
