...
export function PlayerCharacters({
  ...
  useEffect(() => {
    async function loadData() {
      try {
        await ensureDefaultCampaign(CAMPAIGN_ID);
        const loadedCharacters = await loadCharacters(CAMPAIGN_ID);
  ...
  const addCharacter = async (character: PlayerCharacter) => {
    ...
    // Salva su Supabase
    try {
      await saveCharacterToSupabase(CAMPAIGN_ID, character);
    } catch (error) {
  ...
  const updateCharacter = async (id: string, updatedChar: PlayerCharacter) => {
    ...
    // Salva su Supabase
    try {
      await saveCharacterToSupabase(CAMPAIGN_ID, updatedChar);
    } catch (error) {
  ...
  setCharacters(prev =>
    prev.map(char => {
      ...
      // Salva su Supabase
      saveCharacterToSupabase(CAMPAIGN_ID, updated).catch(error => {
  ...
  <div className="min-h-screen bg-[var(--dash-bg)] text-[var(--dash-text-strong)]">
    <div className="mx-auto max-w-7xl px-6 py-8">
      <div className="mb-6 flex items-center justify-between border-b border-[var(--dash-border-soft)] pb-4">
        <div>
          <h2 className="text-3xl font-semibold tracking-[0.08em] uppercase text-[var(--dash-text-strong)]">
            Personaggi Giocanti
          </h2>
