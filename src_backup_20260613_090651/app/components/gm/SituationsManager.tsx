...
export function SituationsManager({
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const loadedSituations = await loadSituations(DEFAULT_CAMPAIGN_ID);
  ...
  const addSituation = async () => {
    ...
    try {
      ...
      await saveSituationToSupabase(DEFAULT_CAMPAIGN_ID, newSituation);
    } catch (error) {
  ...
  const saveSituationChanges = async (situation: Situation) => {
    try {
      // Passa l'intero oggetto - il servizio si occupa del mapping automatico
      await saveSituationToSupabase(DEFAULT_CAMPAIGN_ID, situation);
    } catch (error) {
